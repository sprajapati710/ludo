...
router.get('/gameResult', (req, res) => {
    ...
    const sql = `select * from BOARD_MAPPING where board_id = ?`;
    var results = sync_db.query(sql, [board_id]);
    ...
    const updateSql = 'update BOARD_MAPPING set rank = ? where board_id = ? and color = ?';
    var updateResult = sync_db.query(updateSql, [rank, board_id, color]);
    if(updateResult) {
      return res.status(200).json("Updated rank!").end();
    }
  }
});
});
